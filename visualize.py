...
def visualize_layer(model,
    ...
    def _generate_filter_image(input_img,
        ...
        input_img_data = cv2.resize(imageToUse, (intermediate_dim[0], intermediate_dim[1]))
        if K.image_data_format() == 'channels_first':
            # input_img_data = np.random.random(
            #     (1, 3, intermediate_dim[0], intermediate_dim[1]))
            input_img_data = np.resize(input_img_data, (1, 3, intermediate_dim[0], intermediate_dim[1]))
        else:
            # input_img_data = np.random.random(
            #     (1, intermediate_dim[0], intermediate_dim[1], 3))
            input_img_data = np.resize(input_img_data, (1, intermediate_dim[0], intermediate_dim[1], 3))
    ...
    filter_upper = (filter_range[1]
                    ...
                    # else len(output_layer.get_weights()))
                    else len(output_layer.get_weights()[0][0][0]))
    assert(filter_lower >= 0
           and filter_upper <= len(output_layer.get_weights()[0][0][0])
        #    and filter_upper <= len(output_layer.get_weights())
...
if __name__ == '__main__':
    ...
    visualize_layer(mobilenet, LAYER_NAME, output_dim=(300, 300))